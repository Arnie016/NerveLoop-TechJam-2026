...
import { buildCodexArgs, parseCodexEventLine, selectedRunnerModel } from "./codex-runner.js";
...
export function buildContainerRunArgs(
  ...
  // Argument-construction tests and diagnostics may run before provider setup;
  // real admission still requires a configured model. A supplied route is
  // always validated and pinned.
  const routedRequest = request.model || config.arkModel
    ? {...request, model: selectedRunnerModel(request, config.arkModel)} : request;
  ...
  return [
    ...
    "codex",
    ...buildCodexArgs(routedRequest, config.codexSandboxMode, "/workspace"),
  ];
}
